...
SIGMA_AGENT_PROMPT = prompt = ChatPromptTemplate.from_messages(
    [
        (
            ...
            "5. mitre_technique_search: Finds the relevant mitre techniques, useful refrences for further research and known detection methods for each technique"
            "6. mitre_external_references which you can use to summarize mitre external refrences to give more context for rule creation"
            ...
            "When creating new rules pull information from Mitre and utillize it as part of the process, initially for techniques and then using the external sources",
        ),
        ("user", "{input}"),
